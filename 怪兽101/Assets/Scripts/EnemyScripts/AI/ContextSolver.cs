...
public class ContextSolver : MonoBehaviour
{
    ...
    public Vector2 GetDirectionToMove(List<SteeringBehaviour> behaviours, AIData aiData)
    {
        ...
        //Loop thru each behaviour
        foreach(SteeringBehaviour behaviour in behaviours)
        {
            (danger, interest) = behaviour.GetSteering(danger, interest, aiData);
        }

        //subtract danger value from interest, this will remove all the other directions we dont want to move in incase we are very close to an obstacle
        for (int i = 0; i < 8; i++)
        {
